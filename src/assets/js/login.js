...
$("#email").focusout(function(){
    var request = $.ajax({
            url: "http://localhost:8000/login/email",
            type: "GET",
            async: false,
            data: {
...
nextBtn.addEventListener("click", () => {
    ...
    if(current_step == 0){
        if($("#email").val() != '' && $("#password") != ''){
            var formdata = {
            username: changetext($("#email").val()),
            password: changetext($("#password").val())
        }
        var request = $.ajax({
        ...
        request.fail(function(jqXHR,textStatus,errorMessage){
            ...
            $(".pwderror").html(msg.detail);
        });
    }else{
        $(".pwderror").html("Please enter the mandatory field");
    }
        ...
        var request = $.ajax({
            url: "http://localhost:8000/login/otp",
            headers: {"Authorization": "Bearer "+token},
            type: "POST",
            async: false,
            data: formdata,
        ...
        request.done(function(data){
            ...
            previous_step = current_step-1;
            var imgreq = $.ajax({
                url: "http://localhost:8000/randomimage",
                type: "GET",
                async: false,
                data: {
                    userid: changetext($("#email").val())
    ...
    if (statuscode == 200 && current_step > 0 && current_step <= stepCount) {
...
submitBtn.addEventListener("click", () => {
    ...
    $('input[type="checkbox"]:checked').each(function(index){
        var id = $(this).attr('id');
        imgfile.push($('label[for="'+id+'"]').find('img').attr('src'));
    });*/
    ...
    if(imgfile.length == 0 && blob === undefined){
        ...
        var request = $.ajax({
            url: "http://localhost:8000/verifyimage",
            headers: {"Authorization": "Bearer "+token},
            type: "POST",
            async: false,
            data: formdata,
